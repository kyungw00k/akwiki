...
- **일지 (`akwiki note`)** — CLI에서 빠르게 기록, 연도/월별 자동 정리
...
| `akwiki note "텍스트"` | 일지 작성 — 연도/월별 페이지 자동 생성 (`n`, `log` 별칭) |
...
CLI에서 빠르게 기록하면 연도/월별 페이지에 자동 정리됩니다.

```bash
akwiki note "OAuth2 리다이렉트 URI 설정 방법 정리"
akwiki n "회의: 다음 스프린트 목표 합의"   # 별칭
```
...
```
pages/
  Journal 2026.md    ← 연도 인덱스 (월별 링크)
  2026-04.md         ← 월별 일지 (날짜 + 시간 헤딩)
```
